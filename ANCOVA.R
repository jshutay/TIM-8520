...
#Testing homogeneity of regression slopes (assuming MediatorDomain is the covariate)
RegressionGender <- aov(LeadershipData$ValuesDomain ~ LeadershipData$MediatorDomain*LeadershipData$Gender)
summary(RegressionGender)

RegressionEmployment <- aov(LeadershipData$ValuesDomain ~ LeadershipData$MediatorDomain*LeadershipData$Employment)
summary(RegressionEmployment)

#View the results of the homogeneity of regression slopes
ancova(ValuesDomain ~ MediatorDomain + Gender, data=LeadershipData)
ancova(ValuesDomain ~ MediatorDomain + Employment, data=LeadershipData)

#Run hypothesis/significance tests

#Factorial ANOVAs
FactorialModel<-aov(ValuesDomain ~ Gender*Employment + MediatorDomain, data = LeadershipData)
Ancova(FactorialModel, type="III")
